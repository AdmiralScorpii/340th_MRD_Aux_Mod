...
//_FirstAdd= _VIVLHD execVM "ATLAS_Water\LHD\scripts\addCargoUnloadEH.sqf";
	...
	for "_j" from 0 to ((count (getVehicleCargo _VIVLHD)) -1) do 
	{
		...
		_safeVehicleGetout = _vehicleCargos addEventHandler ["CargoUnloaded", {_vehicleCargos execVM "ATLAS_Water\LHD\scripts\VehicleSafeGetOut.sqf"}];
		for "_i" from 0 to ((count crew _vehicleCargos) -1) do
		{
			_LoadedMan = crew _vehicleCargos select _i;
			_safeGetout = _LoadedMan addEventHandler ["getOutMan", {_LoadedMan execVM "ATLAS_Water\LHD\scripts\SafeGetOut.sqf"}];
		};
	};
